...
test("BDD-REL-304 provisions Production SMTP through protected OIDC authority", async () => {
  ...
  assert.match(workflow, /Y7_PRODUCTION_SMTP_ENTRA_CLIENT_ID/u);
  assert.match(workflow, /Y7_PRODUCTION_SMTP_ENTRA_PRINCIPAL_ID/u);
  assert.match(
    workflow,
    /entraApplicationId="\$\{\{ vars\.Y7_PRODUCTION_SMTP_ENTRA_CLIENT_ID \}\}"/u,
  );
  assert.doesNotMatch(workflow, /az ad sp show/u);
  assert.match(
    workflow,
    /PRINCIPAL_ID="\$\{\{ vars\.Y7_PRODUCTION_SMTP_ENTRA_PRINCIPAL_ID \}\}"/u,
  );
  assert.match(workflow, /Communication and Email Service Owner/u);
  assert.match(workflow, /role assignment list/u);
  assert.doesNotMatch(workflow, /role assignment create/u);
...
test("BDD-E2E-301 runs the complete G5 evidence pack with ephemeral secret material", async () => {
  ...
  assert.match(workflow, /PROVIDER_OUTBOX_TRIGGER_SECRET=%s/u);
  assert.match(workflow, /unset PROVIDER_OUTBOX_TRIGGER_SECRET/u);
  assert.match(workflow, /printf '\\nRELEASE=%s\\nPROVIDER_OUTBOX_TRIGGER_SECRET=%s/u);
...
test("BDD-REL-302 deploys the Production scanner only through protected OIDC", async () => {
  ...
  assert.match(workflow, /monitor metrics alert show/u);
  assert.match(workflow, /PRODUCTION_ALERT_ROUTE_CONFIGURED_AZURE_STUDENT/u);
  assert.doesNotMatch(workflow, /containerapp (?:stop|start)/u);
...
test("BDD-REL-406 stages, promotes, rolls back and restores Production", async () => {
  ...
  assert.match(workflow, /await_web_release "\$PRODUCTION_WEB_ORIGIN" "\$GITHUB_SHA"/u);
  assert.match(workflow, /PRODUCTION_WEB_RELEASE_CONVERGENCE_TIMEOUT/u);
