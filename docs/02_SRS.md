...
| FR-PRIV-009 | PD-012 | Before purge, an authorized workspace actor with the restoration capability MUST be able to restore soft-deleted Feedback through an audited operation; restoration MUST NOT recreate information already anonymized. | Authorized pre-purge restoration returns the item to ordinary scope with an audit entry, unauthorized or post-purge restoration fails, and anonymized identity remains unavailable. |
| NFR-REC-001 | PD-018 | The system MUST create a recoverable backup at least daily and retain each backup for 30 days. | Backup inventory over a 30-day observation shows no gap greater than 24 hours and no backup retained past the 30-day policy. |
