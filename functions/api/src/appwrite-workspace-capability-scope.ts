...
function validateSchema(schema: AppwriteWorkspaceCapabilityScopeSchema): void {
  const ids = [
    schema.databaseId,
...
export function createAppwriteWorkspaceCapabilityScopeResolver(
  tables: AppwriteWorkspaceScopeTablesPort,
  schema: AppwriteWorkspaceCapabilityScopeSchema,
  ...
  return {
    async resolve(input) {
      ...
      try {
        ...
        return policy.can(actor, input.capability, project)
          ? { status: "authorized", actor, project }
          : { status: "denied" };
      } catch (error: unknown) {
...
export function createNodeAppwriteWorkspaceCapabilityScopeResolver(
  tables: TablesDB,
  schema: AppwriteWorkspaceCapabilityScopeSchema,
): WorkspaceCapabilityScopeResolver {
  return createAppwriteWorkspaceCapabilityScopeResolver(
    {
